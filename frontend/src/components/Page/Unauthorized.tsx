...
export default function Unauthorized() {
  ...
  return (
    <main className="relative flex min-h-screen w-full flex-col bg-page-dark overflow-hidden font-sans">
      {/* Fond avec dégradé similaire aux autres pages */}
      <section className="absolute inset-0 bg-connexion opacity-80 pointer-events-none"></section>

      {/* Bouton Retour en haut à gauche */}
      <section className="absolute left-6 top-8 z-10 lg:left-10 lg:top-10">
        <button
          onClick={() => navigate(-1)}
      ...
      <section className="relative z-10 flex flex-1 flex-col items-center justify-center px-4 text-center">
        <section className="flex flex-col items-center max-w-3xl gap-12">
      ...
      <section className="absolute top-1/4 right-0 lg:right-24 h-64 w-64 opacity-50 blur-sm pointer-events-none hidden md:block">
         <section className="w-full h-full bg-primary/20 rounded-full blur-3xl"></section>
      </section>
    </main>
  );
}
